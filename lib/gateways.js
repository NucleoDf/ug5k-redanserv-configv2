...
exports.insertTfnoRes4Gateway = function insertTfnoRes4Gateway(tr, f){
	...
	connection.connect(function(err){
		if (err){
			...
			var querySel = connection.query('INSERT INTO recursos_telefono (pasarela_id,numero_ia4,posicion_ia4,nombre) ' +
				'VALUES (?,?,?,?) ',
				[tr.pasarela_id,tr.numero_ia4,tr.posicion_ia4,tr.nombre],
				function(err, result) {
					logging.LoggingDate(querySel.sql);
					connection.end();
					if (err)
...
exports.getHardwareResume = function getHardwareResume(idGtw, f){
	...
	connection.connect(function(err){
		if (err){
			...
			var queryRadio = connection.query('SELECT rr.idrecurso_radio, rr.numero_ia4, rr.posicion_ia4, ' +
				...
				function(err, rows) {
					...
					if (err) {
						...
						recursosRadio=rows;
						var queryTfno = connection.query('SELECT rt.idrecurso_telefono, rt.numero_ia4, rt.posicion_ia4, rt.nombre ' +

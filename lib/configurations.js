...
exports.postConfigurationFromJsonFile = function(idcfg,idSite,newCgwData, f) {
	...
	connection.connect(function (err) {
		if (err) {
			...
			async function insertGtwIpListData(newInsertCgwId, flagResources, resRadioId, callback) {
				var queryString = '';
				var i=0;
			...
			async function insertGtwRadioUriListData(newInsertCgwId, flagResources, resRadioId, callback) {
				var queryString = '';
				if(flagResources.resRadio) {
					for (var i=0;i<newCgwData.recursos.length;i++) {
						if(newCgwData.recursos[i].Radio_o_Telefonia==1) {

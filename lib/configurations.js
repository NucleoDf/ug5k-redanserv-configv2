...
exports.getConfiguration_new = function getConfiguration(req, res, cfg) {
    var mySql = require('mySql');
    var connection = ug5kdb.GetDbConnection();/* mySql.createConnection({
	  host     : gcfg.Ulises.dbhost,
	  user     : gcfg.Ulises.dbuser,
    ...
    connection.connect(function (err) {
        ...
        //    'LEFT JOIN emplazamientos e ON c.idconfiguracion = e.configuracion_id ' +
        //    'LEFT JOIN pasarelas p ON e.idemplazamiento = p.emplazamiento_id ' +
        //    'WHERE c.idconfiguracion = ?' + ' ORDER BY ultima_actualizacion DESC';
        var ultima_actualizacion = async function (idconfig) {
            //console.log("insertNewSites entry " + newIdCfg);
            var query = 'SELECT MAX(p.ultima_actualizacion) FROM configuraciones c ' +
                'LEFT JOIN emplazamientos e ON c.idconfiguracion = e.configuracion_id ' +
                'LEFT JOIN pasarelas p ON e.idemplazamiento = p.emplazamiento_id ' +
                'WHERE c.idconfiguracion = ' + idconfig;
            res = await ug5kdb.QuerySync(query);
            console.log(res);
            return "ultima_actualizacion!!!!";
        };
        var ua = ultima_actualizacion(cfg);

        var strQuery = 'SELECT DISTINCT c.idconfiguracion as idCFG, c.nombre as name, ' +
            'c.descripcion as description, c.activa as activa, ' +
            'DATE_FORMAT(c.activa, "%d/%m/%Y %H:%i:%s") as ts_activa, ' +
            'e.nombre as nameSite, e.idemplazamiento as idEMPLAZAMIENTO, ' + '"' + ua + '" as ultima_actualizacion ' +
            'FROM configuraciones c ' +
            'LEFT JOIN emplazamientos e ON c.idconfiguracion = e.configuracion_id ' +
            'LEFT JOIN pasarelas p ON e.idemplazamiento = p.emplazamiento_id ' +
            'WHERE c.idconfiguracion = ?' + ' ORDER BY ultima_actualizacion DESC';
    ...
    connection.connect(function (err) {
        if (err) {
            ...
            var insertNewCfg = function /*insertNewCfg*/(callback) {
                //console.log("insertNewCfg entry");
                var query = connection.query('INSERT INTO configuraciones ' +
                    ...
                    function (err, rows) {
                        ...
                        if (err || rows.length == 0) {
                            ...
                            //console.log("insertNewCfg entry end ok");
                            return callback(null);
                        }
                    });
            };
            var selectSitesFromCfg = function /*selectSitesFromCfg*/(callback) {
                //console.log("selectSitesFromCfg entry " + newIdCfg);
                var query = connection.query('SELECT e.idemplazamiento,e.nombre ' +
                    ...
                    function (err, rows) {
                        ...
                        if (err || rows.length == 0) {
                            ...
                            oldSiteData = rows;
                            //console.log("selectSitesFromCfg endok " + newIdCfg);
                            return callback(null);
                        }
                    });
            };
            var insertNewSites = async function /*insertNewSites*/(callback) {
                ...
                //console.log("insertNewSites endok " + newIdCfg);
                return callback(null);
            };
            var selectGateways = function /*selectGateways*/(callback) {
                //console.log("selectGateways entry " + newIdCfg);
                var query = connection.query('SELECT p.* ' +
                    ...
                    function (err, rows) {
                        ...
                        if (err || rows.length == 0) {
                            ...
                            oldGtwData = rows;
                            //console.log("selectGateways endok " + newIdCfg);
                            return callback(null);
                        }
                    });
            };
            var insertNewGtws = async function /*insertNewGtws*/(callback) {
                //console.log("insertNewGtws entry " + newIdCfg);
                var query = '';
                var res = '';
                var empId2Insert = 0;
                ...
                //console.log("insertNewGtws endok " + newIdCfg);
                return callback(null);
            };
            var selectGatewayIps = function /*selectGatewayIps*/(callback) {
                //console.log("selectGatewayIps entry " + newIdCfg);
                var query = connection.query('SELECT li.* ' +
                    ...
                    function (err, rows) {
                        ...
                        if (err || rows.length == 0) {
                            ...
                            oldGtwIpData = rows;
                            //console.log("selectGatewayIps endok " + newIdCfg);
                            return callback(null);
                        }
                    });
            };
            var insertNewGtwIps = async function /*insertNewGtwIps*/(callback) {
                ...
                var res = '';
                var gtwId2Insert = 0;
                //console.log("insertNewGtwIps entry " + newIdCfg);
                ...
                //console.log("insertNewGtwIps endok " + newIdCfg);
                return callback(null);
            };
            var selectGatewayRadioRes = function /*selectGatewayRadioRes*/(callback) {
                //console.log("selectGatewayRadioRes entry " + newIdCfg);
                var query = connection.query('SELECT rr.* ' +
                    ...
                    function (err, rows) {
                        ...
                        if (err || rows.length == 0) {
                            ...
                            //console.log("selectGatewayRadioRes endok " + newIdCfg);
                            return callback(null);
                        }
                    });
            };
            var insertNewGtwRadioRes = async function /*insertNewGtwRadioRes*/(callback) {
                ...
                var res = '';
                var gtwId2Insert = 0;
                //console.log("insertNewGtwRadioRes entry " + newIdCfg);
                ...
                //console.log("insertNewGtwRadioRes endok " + newIdCfg);
                return callback(null);
            };
            var selectGatewayTfnoRes = function /*selectGatewayTfnoRes*/(callback) {
                //console.log("selectGatewayTfnoRes entry " + newIdCfg);
                var query = connection.query('SELECT rt.* ' +
                    ...
                    function (err, rows) {
                        ...
                        if (err || rows.length == 0) {
                            ...
                            //console.log("selectGatewayTfnoRes endok " + newIdCfg);
                            return callback(null);
                        }
                    });
            };
            var insertNewGtwTfnoRes = async function /*insertNewGtwTfnoRes*/(callback) {
                ...
                var res = '';
                var gtwId2Insert = 0;
                //console.log("insertNewGtwTfnoRes entry " + newIdCfg);
                ...
                //console.log("insertNewGtwTfnoRes endok " + newIdCfg);
                return callback(null);
            };
            var selectRadioResLista_Uris = function /*selectRadioResLista_Uris*/(callback) {
                //console.log("selectRadioResLista_Uris entry " + newIdCfg);
                var query = connection.query('SELECT lu.* ' +
                    ...
                    function (err, rows) {
                        ...
                        if (err || rows.length == 0) {
                            ...
                            //console.log("selectRadioResLista_Uris endok " + newIdCfg);
                            return callback(null);
                        }
                    });
            };
            var insertNewRadioResLista_Uris = async function /*insertNewRadioResLista_Uris*/(callback) {
                ...
                var res = '';
                var rrId2Insert = 0;
                //console.log("insertNewRadioResLista_Uris entry " + newIdCfg);
                ...
                //console.log("insertNewRadioResLista_Uris endok " + newIdCfg);
                return callback(null);
            };
            var selectTfnoResRangos_Ats = function /*selectTfnoResRangos_Ats*/(callback) {
                //console.log("selectTfnoResRangos_Ats entry " + newIdCfg);
                var query = connection.query('SELECT ra.* ' +
                    ...
                    function (err, rows) {
                        ...
                        if (err || rows.length == 0) {
                            ...
                            //console.log("selectTfnoResRangos_Ats endok " + newIdCfg);
                            return callback(null);
                        }
                    });
            };
            var insertNewTfnoResRangos_Ats = async function /*insertNewTfnoResRangos_Ats*/(callback) {
                ...
                var res = '';
                var rtId2Insert = 0;
                //console.log("insertNewTfnoResRangos_Ats entry " + newIdCfg);
                ...
                //console.log("insertNewTfnoResRangos_Ats endok " + newIdCfg);
                return callback(null);
            };
            // CODIGO ASÍNCRONO
            async.waterfall([

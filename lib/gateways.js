	...
	connection.connect(function (err) {
		if (err) {
			...
			query = connection.query('UPDATE pasarelas SET nombre=?,ip_virtual=?,ip_cpu0=?,ip_gtw0=?,' +
				'mask_cpu0=?,ip_cpu1=?,ip_gtw1=?,mask_cpu1=? WHERE idpasarela=?',
				[gtw.nombre,gtw.ipv,gtw.ipb1,gtw.ipg1,gtw.msb1,gtw.ipb2,gtw.ipg2,gtw.msb2,idGtw],
				function(err, rows) {
					logging.LoggingDate(query.sql);
					if (err) {
						return f({error: err.message});
						...
						function(err, rows) {
							...
							if (err) {
								...
								if (lista_ips.length > 0) {
									...
									lista_ips.forEach(function (item) {
										queries += mySql.format('INSERT INTO lista_ips (pasarela_id,ip,puerto,tipo) ' +
											'VALUES (?,?,?,?); ', [idGtw, item.ip, item.puerto, item.tipo]);
									});
									connection.query(queries, function (err, results) {
										connection.end();
										if (err) {

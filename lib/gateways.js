...
exports.getIpv2 = function getIpv(ipBound,f) {
	...
	connection.connect(function (err, usrs) {
		if (err) {
			...
			query = connection.query('SELECT c.nombre as idConf, c.activa, ' +
					...
					if (err!=null){
						...
						if (rows == null || rows.length == 0) {//No en BBDD
							...
							if(rows[0].activa==0)//No en config Activa
								...
								f({error:null, toLocal:null, data:{idConf:rows[0].idConf,
									fechaHora:rows[0].fechaHora+' UTC', idGtw:rows[0].idpasarela}, ipv:-1})
						}
					}
				});
		}
	...
	connection.connect(function(err, usrs) {
		if (err) {
			...
			var query = connection.query('SELECT COUNT(*) as cuantos ' +
				'FROM pasarelas p ' +
				'WHERE p.idpasarela=? ' +
				'AND p.nombre LIKE (SELECT p.nombre ' +
				'FROM pasarelas p ' +
...
/*  REV 1.0.2 VMG										*/
/********************************************************/
exports.checkGatewayNameIp = function (sourceIdGateway, targetNameGateway, ipTargetGateway, f) {
	var mySql = require('mySql');
	var connection = mySql.createConnection({
		host: gcfg.Ulises.dbhost,
		user: gcfg.Ulises.dbuser,
	...
	connection.connect(function (err) {
		if (err) {
			...
			/// Comprobar que el nombre de la pasarela no exista ya en la configuracion
			var querySelect = connection.query('SELECT p.nombre,p.ip_cpu0,p.ip_cpu1 ' +
				...
				if (err != null)
					...
					for(var i=0;i<rows.length;i++){
						...
						if(rows[i].ip_cpu0==ipTargetGateway) {
							return f({error: 'ER_DUP_IP_ENTRY'});
						}
						if(rows[i].ip_cpu1==ipTargetGateway) {
							return f({error: 'ER_DUP_IP_ENTRY'});
						}
					}
					return f({data: 'OK'});
...
/*  REV 1.0.2 VMG										*/
/********************************************************/
exports.copyGateway = function (sourceIdGateway, targetNameGateway, ipTargetGateway, f){
	var mySql = require('mySql');
	var connection = mySql.createConnection({
	  host     : gcfg.Ulises.dbhost,
	  user     : gcfg.Ulises.dbuser,
	...
	connection.connect(function(err){
		if (err){
			...
			/// Comprobar que el nombre de la pasarela no exista ya en la configuracion
			var querySelect = connection.query('SELECT * '+
			'FROM pasarelas p '+
			'LEFT JOIN emplazamientos e ON p.emplazamiento_id=e.idemplazamiento '+
			'LEFT JOIN configuraciones c ON e.configuracion_id=c.idconfiguracion '+
			'WHERE p.idpasarela=?',
			...
			function(err,rows){
				...
				if (err != null) {
					...
					//TODO vamos por aqui   VMG
					var queryInsert =
					connection.query('INSERT INTO pasarelas ' +
					'(emplazamiento_id,nombre,ip_virtual,ultima_actualizacion,ip_cpu0,ip_gtw0,mask_cpu0, ' +
					'ip_cpu1,ip_gtw1,mask_cpu1)' +
					//', puerto_sip, periodo_supervision, puerto_servicio_snmp, puerto_snmp, snmpv2, ' +
					//'comunidad_snmp, nombre_snmp, localizacion_snmp, contacto_snmp, puerto_servicio_web, ' +
					//'tiempo_sesion, puerto_rtsp, servidor_rtsp, servidor_rtspb)'
					'VALUES ' +
					'(?,?,?, NOW(),?,?,?,?,?,?)', [rows[0].emplazamiento_id,targetNameGateway,rows[0].ip_virtual,
					ipTargetGateway,rows[0].ip_gtw0,rows[0].mask_cpu0,rows[0].ip_cpu1,rows[0].ip_gtw1,rows[0].mask_cpu1],
					function (err, resultCgw) {
...
exports.getAll = function getAll(configName,ipGtw, f) {
	...
	connection.connect(function (err) {
		if (err) {
			...
			function selectDataPasarelas(idGtw,callback) {
			...
			function selectDataTfno(idGtw, template, callback) {
						...
						if (err || rows.length == 0){
							...
							for(var i=0;i<rows.length;i++) {
								...
								if(rows[i].ajuste_ad==null) {
									...
									recurso.AD_GAIN = rows[i].ajuste_ad;
									recurso.AD_AGC = 0;
								}
								if(rows[i].ajuste_da==null) {
									...
									recurso.DA_GAIN = rows[i].ajuste_da;
									recurso.DA_AGC = 0;
								}
			...
			function selectDataRadio(idGtw, template, callback) {
				var query=connection.query('SELECT idrecurso_radio,nombre,columna,fila,codec,clave_registro,ajuste_ad, ' +
					...
					'cola_bss_sqh,evento_ptt_squelch,retardo_jitter,retraso_interno_grs,habilita_grabacion,prioridad_sesion_sip,prioridad_ptt,' +
					...
					function(err, rows) {
						...
						if (err || rows.length == 0){
							...
							for(var i=0;i<rows.length;i++) {
								...
								recurso.radio.iSesionPrio = rows[i].prioridad_ptt;
								recurso.radio.colateral.name = rows[i].frecuencia.toString();
			...
			function selectListaIps(idGtw, template, callback) {
			...
			function selectListaUris(idGtw, template, callback) {
			...
			function selectListaRangosAts(idGtw, template, callback) {
			...
			function selectListaUsers(idGtw, template, callback) {

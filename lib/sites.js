...
/*  REV 1.0.2 VMG					*/
/************************************/
exports.putSite = function putSite(idSite,idCfg,nameSite,f){
	var mySql = require('mySql');
	var connection = mySql.createConnection({
	  host     : gcfg.Ulises.dbhost,
	  user     : gcfg.Ulises.dbuser,
	...
	connection.connect(function(err, usrs){
		if (err){
			...
			logging.LoggingDate("Successful connection to 'U5K-G' database!");
			var querySelect = connection.query('SELECT COUNT(*) AS cuantos FROM emplazamientos ' +
							'WHERE nombre=? AND configuracion_id=? ', [nameSite, idCfg],
			function (err, rows) {
				logging.LoggingDate(querySelect.sql);
				if (err) {
					connection.end();
					...
					connection.end();
					return f({error: 'ER_DUP_ENTRY', dupNmae: nameSite});
				}
				else {

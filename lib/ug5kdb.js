...
function Query(query, param1, cbRes, nolog) {
    ...
    var connection = GetDbConnection(); /* mySql.createConnection({
    ...
    logging.Trace(BuildStrQuery(QueryCount, query, param1));
    connection.connect(function(mysqlerr) {
        if (mysqlerr) {
            logging.Trace(BuildStrResult(mysqlerr));
            cbRes(mysqlerr);
            connection.end();
        }
        else {
            connection.query(query, param1, function(mysqlerr, rows, fields) {
                connection.end();
                logging.Trace(BuildStrResult(QueryCount, mysqlerr, rows));

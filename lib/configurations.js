...
exports.copyConfiguration = function copyConfiguration(idSourceConfig, trgt, f){
	...
	connection.connect(function(err){
		if (err){
			...
			function insertNewSites(callback) {
				var queries='';
				oldSiteData.forEach(function (item) {
					queries += mySql.format('INSERT INTO emplazamientos (nombre, configuracion_id) ' +
					'VALUES (?,?);',[item.nombre,newIdCfg]);
				});

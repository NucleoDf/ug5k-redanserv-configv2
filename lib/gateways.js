						...
						if (err){
							...
							for(var i=0;i<rows.length;i++) {
								...
								if(rows[i].ajuste_ad==null) {
									...
									recurso.AD_GAIN = rows[i].ajuste_ad*10;//Al mandarlo a las pasarelas hay que multiplicarlo
									recurso.AD_AGC = 0;
								}
								if(rows[i].ajuste_da==null) {
									recurso.DA_GAIN = '';
									...
									recurso.DA_GAIN = rows[i].ajuste_da*10;//Al mandarlo a las pasarelas hay que multiplicarlo
									recurso.DA_AGC = 0;
								}
			...
			function(err, rows, fields) {
				...
				if (err!=null){
					...
					if (rows == null || rows.length == 0) {//No en BBDD
						...
						for(var i=0;i<rows.length;i++){
							if(rows[i].activa==1) {
								f({error:null, toLocal:null, data:{idConf:rows[i].idConf,
									fechaHora:rows[i].fechaHora+' UTC', idGtw:rows[i].idpasarela,
										updatePend:rows[i].pendiente_actualizar}, ipv:-1})
								return;
							}
							else {
								f({error:null, toLocal:-2, data:{idConf:rows[i].idConf,
									fechaHora:rows[i].fechaHora+' UTC', idGtw:rows[i].idpasarela,
										updatePend:rows[i].pendiente_actualizar}, ipv:-1})
								return;
							}
...
exports.copyGateway = function (sourceIdGateway,targetNameGateway,
	...
	connection.connect(function(err){
		if (err){
			...
			function selectGateway(callback) {
			...
			async function insertNewGtw(callback) {
				...
				for(var i=0;i<oldGtwRadioResData.length;i++) {
					if(oldGtwRadioResData[i].pasarela_id!=null) {
						query = 'INSERT INTO recursos_radio (pasarela_id,fila,columna,nombre,codec,' +
							...
							');';
						res = await
						ug5kdb.QueryMultiInsertSync(query);
	...
	connection.connect(function (err) {
		if (err) {
			...
			query = connection.query('INSERT INTO pasarelas (emplazamiento_id,nombre,ip_virtual,ip_cpu0,ip_gtw0,' +
									...
									'puerto_rtsp,servidor_rtsp,servidor_rtspb,ultima_actualizacion) ' +
									'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())',
	...
	connection.connect(function (err) {
		if (err) {
			...
			query = connection.query('UPDATE pasarelas SET nombre=?,ip_virtual=?,ip_cpu0=?,ip_gtw0=?,' +
				'mask_cpu0=?,ip_cpu1=?,ip_gtw1=?,mask_cpu1=?,puerto_snmp=?,puerto_servicio_snmp=?,' +
				'periodo_supervision=?,snmpv2=?,comunidad_snmp=?,puerto_servicio_web=?,tiempo_sesion=?, ' +
				'puerto_rtsp=?,servidor_rtsp=?,servidor_rtspb=?,pendiente_actualizar=1 WHERE idpasarela=?',

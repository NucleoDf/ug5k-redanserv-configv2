...
exports.getConfigurations = function getConfigurations(req, res){
	...
	connection.connect(function(err, usrs){
		if (err){
			...
			var query = connection.query('SELECT idconfiguracion as idCFG, nombre as name, descripcion,' +
				'region, activa, fecha_activacion ' +
	...
	connection.connect(function(err){
		...
		var query = connection.query('SELECT c.idconfiguracion as idCFG, c.nombre as name, '+
			...
			if(err)
				...
				if(rows.length == 1 && rows[0].nameSite == null)
					res.status(202).json("Site not found.");
				else
					res.json({result:c});
		    }
		    else
					...
					if(err)
						...
						function (err, rows, result) {
							if(err)
								...
								var query = connection.query('CALL SP_Delete_Sincr(?)',[cfg],
								function (err, rows, result) {
									if(err)
										return callback(null, 'ERROR');
									else {
										var query = connection.query('CALL SP_Delete_Grab(?)',[cfg],
										function (err, rows, result) {
											if(err)
												return callback(null, 'ERROR');
											else {
												var query = connection.query('CALL SP_Delete_Snmp(?)',[cfg],
												function (err, rows, result) {
													if(err)
														return callback(null, 'ERROR');
													else {
														var query = connection.query('CALL SP_Delete_Web(?)',[cfg],
														function (err, rows, result) {
															if(err)
																return callback(null, 'ERROR');
															else {
																var query = connection.query('CALL SP_Delete_Sip(?)',[cfg],
																function (err, rows, result) {
																	if(err)
																		return callback(null, 'ERROR');
																	else {
																		var query = connection.query('CALL SP_Delete_Servicios(?)',[cfg],
																	 	function (err, rows, result) {
																			if(err)
																				return callback(null, 'ERROR');
																			else{
																				var query = connection.query('CALL SP_Delete_Cgws(?)',[cfg],
																				function (err, rows, result) {
																					if(err)
																						return callback(null, 'ERROR');
																					else {
																						var query = connection.query('CALL SP_Delete_Sites(?)',[cfg],
																						function (err, rows, result) {
																							if(err)
																								return callback(null, 'ERROR');
																							else {
																								var query = connection.query('CALL SP_Delete_Cfgs(?)',[cfg],
																								function (err, rows, result) {
																									if(err)
																										return callback(null, 'ERROR');
																									else

...
exports.insertRadioRes4Gateway = function insertRadioRes4Gateway(rr, f){
	...
	connection.connect(function(err){
		if (err){
			...
			'frecuencia,tipo_agente,indicacion_entrada_audio,indicacion_salida_audio,umbral_vad,metodo_bss,' +
			'evento_ptt_squelch,prioridad_ptt,prioridad_sesion_sip) ' +
			'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
			[rr.pasarela_id,rr.fila,rr.columna,rr.nombre,rr.frecuencia,rr.tipo_agente,rr.indicacion_entrada_audio,
			rr.indicacion_salida_audio,rr.umbral_vad,rr.metodo_bss,rr.evento_ptt_squelch,rr.prioridad_ptt,
			rr.prioridad_sesion_sip],
			function(err, result) {
				logging.LoggingDate(querySel.sql);
				connection.end();
				if (err)

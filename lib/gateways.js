		...
		if (err){
			...
			if (cfg == null){ //Venimos de ver la gtw al pulsar sobre ella
				query = connection.query('SELECT p.idpasarela as idCGW, p.emplazamiento_id as EMPLAZAMIENTO_idEMPLAZAMIENTO,  '+
										...
										'p.puerto_proxy, p.periodo_supervision, p.puerto_servicio_snmp, p.puerto_snmp, '+
...
exports.getRadioRes4Gateway = function getRadioRes4Gateway(resourceId, f){
	...
	connection.connect(function(err){
		if (err){
			...
			var querySel = connection.query('SELECT rr.nombre, rr.codec, rr.clave_registro, rr.frecuencia, ' +
				'rr.ajuste_ad, rr.ajuste_da, rr.precision_audio '+
...
exports.getTfnoRes4Gateway = function getTfnoRes4Gateway(resourceId, f){
	...
	connection.connect(function(err){
		if (err){
			...
			var querySel = connection.query('SELECT rt.nombre, rt.codec, rt.clave_registro, ' +
				'rt.ajuste_ad, rt.ajuste_da, rt.tipo_interfaz_tel '+
...
exports.insertTfnoRes4Gateway = function insertTfnoRes4Gateway(tr, f){
	...
	connection.connect(function(err){
		if (err){
			...
			var querySel = connection.query('INSERT INTO recursos_telefono (pasarela_id,fila,columna,nombre,tipo_interfaz_tel) ' +
				'VALUES (?,?,?,?,?)',
				[tr.pasarela_id,tr.fila,tr.columna,tr.nombre,tr.tipo_interfaz_tel],
				function(err, result) {
					logging.LoggingDate(querySel.sql);
					connection.end();
					if (err)
...
exports.insertRadioRes4Gateway = function insertRadioRes4Gateway(rr, f){
	...
	connection.connect(function(err){
		if (err){
			...
			var querySel = connection.query('INSERT INTO recursos_radio (pasarela_id,fila,columna,nombre,' +
			'frecuencia,ajuste_ad,ajuste_da,tipo_agente,indicacion_entrada_audio,indicacion_salida_audio,' +
			'umbral_vad,metodo_bss,evento_ptt_squelch,prioridad_ptt,prioridad_sesion_sip) ' +
			'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
			[rr.pasarela_id,rr.fila,rr.columna,rr.nombre,rr.frecuencia,rr.ajuste_ad,rr.ajuste_da,rr.tipo_agente,
			rr.indicacion_entrada_audio,rr.indicacion_salida_audio,rr.umbral_vad,rr.metodo_bss,rr.evento_ptt_squelch,
			rr.prioridad_ptt,rr.prioridad_sesion_sip],
			function(err, result) {
				logging.LoggingDate(querySel.sql);
				connection.end();
				if (err)
					...
					if(lista_ips.length > 0) {
						...
						lista_ips.forEach(function (item) {
							queries += mySql.format('INSERT INTO lista_ips (pasarela_id,ip,puerto,tipo) ' +
								'VALUES (?,?,?,?); ', [rows.insertId, item.ip, item.puerto, item.tipo]);
						});
	...
	connection.connect(function (err) {
		if (err) {
			...
			query = connection.query('INSERT INTO pasarelas (emplazamiento_id,nombre,ip_virtual,ip_cpu0,ip_gtw0,' +
									'mask_cpu0,ip_cpu1,ip_gtw1,mask_cpu1) ' +
									'VALUES (?,?,?,?,?,?,?,?,?)',
			[idSite,gtw.nombre,gtw.ipv,gtw.ipb1,gtw.ipg1,gtw.msb1,gtw.ipb2,gtw.ipg2,gtw.msb2,
			],
			function(err, rows) {
				logging.LoggingDate(query.sql);
				if (err) {
...
function getFormattedIps(gtw){
	...
	for(i=0;i<gtw.traps.length;i++) {
		charIndex=gtw.traps[i].indexOf('/');
		ip2insert.ip=gtw.traps[i].substring(2, charIndex-1);
		ip2insert.puerto=gtw.traps[i].substring(charIndex+1);

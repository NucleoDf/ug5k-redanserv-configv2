...
exports.insertRadioRes4Gateway = function insertRadioRes4Gateway(rr, f){
	...
	connection.connect(function(err){
		if (err){
			...
			rr.retraso_interno_grs,rr.habilita_grabacion,rr.restriccion_entrantes,rr.retardo_fijo_climax],
			function(err, result) {
				logging.LoggingDate(querySel.sql);
				//connection.end();
				if (err)
	...
	connection.connect(function(err){
		if (err){
			...
			var query = connection.query('UPDATE recursos_radio SET nombre=?,clave_registro=?,frecuencia=?,ajuste_ad=?,' +
				...
				'retraso_interno_grs=?,habilita_grabacion=?,restriccion_entrantes=?,retardo_fijo_climax=? ' +
				...
				rr.habilita_grabacion,rr.restriccion_entrantes,rr.retardo_fijo_climax,resId],
				function(err, result) {
					logging.LoggingDate(query.sql);
					//connection.end();
					if (err)

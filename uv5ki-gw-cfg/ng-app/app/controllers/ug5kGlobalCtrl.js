...
ug5kGlobalCtrl.$inject = ['$scope', '$rootScope', '$interval', '$translate', 'dataservice', 'authservice', 'CfgService', 'transerv', 'ngDialog'];

function ug5kGlobalCtrl($scope, $rootScope, $interval, $translate, dataservice, authservice, CfgService, transerv, ngDialog) {
	...
	var sessionTimer = $interval(function () {
	    ...
	    if (++vm.timer >= sesion_sup) {
	        ...
	        if (vm.session == true && authservice.check_session() == false) {
	            ...
	            dataservice.set_data('/logout', {}, false);
	            vm.session = false;
	        }

	        dataservice.get("/tses").then(

...
exports.copyConfiguration = function copyConfiguration(idSourceConfig, trgt, f){
	...
	connection.connect(function(err){
		if (err){
			...
			function insertNewCfg(callback) {
			...
			function selectSitesFromCfg(callback) {
			...
			async function insertNewSites(callback) {
				var query='';
				var res='';
			...
			function selectGateways(callback) {
			...
			async function insertNewGtws(callback) {
				var query='';
				var res='';
				var empId2Insert=0;
			...
			function selectGatewayIps(callback) {
			...
			async function insertNewGtwIps(callback) {
				var query='';
				var res='';
				var gtwId2Insert=0;
			...
			function selectGatewayRadioRes(callback) {
			...
			async function insertNewGtwRadioRes(callback) {
				var query='';
				var res='';
				var gtwId2Insert=0;
			...
			function selectGatewayTfnoRes(callback) {
			...
			async function insertNewGtwTfnoRes(callback) {
				var query='';
				var res='';
				var gtwId2Insert=0;
			...
			function selectRadioResLista_Uris(callback) {
			...
			async function insertNewRadioResLista_Uris(callback) {
				var query='';
				var res='';
				var rrId2Insert=0;
			...
			function selectTfnoResRangos_Ats(callback) {
			...
			async function insertNewTfnoResRangos_Ats(callback) {
				var query='';
				var res='';
				var rtId2Insert=0;
	...
	connection.connect(function(err){
		if (err){
			...
			function(err, rows, fields) {
				logging.LoggingDate(queryHasGateways.sql);
				if (!err && rows != null && rows.length > 0){
					if (rows[0].cuantos == 0){
						...
						var queryUpdate = connection.query('UPDATE configuraciones SET activa=0 WHERE activa=1', function(err, result){
							logging.LoggingDate(queryUpdate.sql);
							if (err){
								connection.end();
...
exports.gatewaysOut = function gatewaysOut(req, res, cfg, f) {
	...
	connection.connect(function (err, usrs) {
		if (err) {
			...
			logging.LoggingDate("Successful connection to 'U5K-G' database!");
			var query = connection.query('SELECT p.nombre as name,p.ip_cpu0,p.ip_cpu1,p.ip_virtual as ipv '+
				'FROM pasarelas p '+
...
exports.postConfigurationFromJsonFile = function(idcfg,idSite,newCgwData, f) {
	...
	connection.connect(function (err) {
		if (err) {
			...
			function insertGtwData(callback) {
				query = connection.query('INSERT INTO pasarelas (emplazamiento_id,nombre,ip_virtual,ultima_actualizacion,' +
			...
			async function insertGtwRadioData(newInsertCgwId, flagResources, callback) {
				var queryString = '';
				if(flagResources.resRadio) {
					for (var i=0;i<newCgwData.recursos.length;i++) {
						if(newCgwData.recursos[i].Radio_o_Telefonia==1) {
							if(newCgwData.recursos[i].AD_AGC==1)
								newCgwData.recursos[i].AD_GAIN='null';
							if(newCgwData.recursos[i].DA_AGC==1)
								newCgwData.recursos[i].DA_GAIN='null';
							...
							''+newCgwData.recursos[i].radio.colateral.name+','+newCgwData.recursos[i].AD_GAIN+',' +
							''+newCgwData.recursos[i].DA_GAIN+','+newCgwData.recursos[i].radio.iPrecisionAudio+',' +
			...
			async function insertGtwPhoneData(newInsertCgwId, flagResources, resRadioId, callback) {
				var queryString = '';
					...
					for (var i=0;i<newCgwData.recursos.length;i++) {
						if(newCgwData.recursos[i].Radio_o_Telefonia==2) {
							...
							queryString = 'INSERT INTO recursos_telefono (pasarela_id,fila,columna,nombre,codec,' +
								...
								'destino_test,supervisa_colateral,tiempo_supervision,duracion_tono_interrup,uri_telefonica) '+
								'VALUES ('+newInsertCgwId+','+newCgwData.recursos[i].NumDispositivoSlot+',' +
								''+newCgwData.recursos[i].SlotPasarela+',"'+newCgwData.recursos[i].IdRecurso+'",' +
								''+newCgwData.recursos[i].Codec+',"'+newCgwData.recursos[i].szClave+'",' +
								''+newCgwData.recursos[i].AD_GAIN+',' +
								''+newCgwData.recursos[i].DA_GAIN+',1,'+newCgwData.recursos[i].telefonia.tipo+',' +
			...
			async function insertGtwIpListData(newInsertCgwId, flagResources, resRadioId, resPhoneId, callback) {
				var queryString = '';
				var i=0;
				...
				for (i=0;i<newCgwData.servicios.snmp.traps.length;i++) {
					if(newCgwData.servicios.snmp.traps[i].ip!='') {
						var version='TRPV1';
						if(newCgwData.servicios.snmp.traps[i].ip.substr(0,1)=='2')
							version='TRPV2';
						queryString = 'INSERT INTO lista_ips (pasarela_id,ip,tipo,selected) ' +
			...
			async function insertGtwRadioUriListData(newInsertCgwId, flagResources, resRadioId, resPhoneId, callback) {
				var queryString = '';
				var k=0;
				if(flagResources.resRadio) {
					for (var i=0;i<newCgwData.recursos.length;i++) {
							...
							for (var j=0;j<newCgwData.recursos[i].radio.colateral.emplazamientos.length;j++) {
								if (newCgwData.recursos[i].radio.colateral.emplazamientos[j].uriTxA != '') {
									queryString = 'INSERT INTO lista_uris (recurso_radio_id,uri,tipo,nivel_colateral) ' +
										'VALUES (' + resRadioId[k] + ',' +
										'"' + newCgwData.recursos[i].radio.colateral.emplazamientos[j].uriTxA + '"' +
										',"TXA",' + j + 1 + ')';
									res = await
								...
								if (newCgwData.recursos[i].radio.colateral.emplazamientos[j].uriTxB != '') {
									queryString = 'INSERT INTO lista_uris (recurso_radio_id,uri,tipo,nivel_colateral) ' +
										'VALUES (' + resRadioId[k] + ',' +
										'"' + newCgwData.recursos[i].radio.colateral.emplazamientos[j].uriTxB + '"' +
										',"TXB",' + j + 1 + ')';
									res = await
									ug5kdb.QueryMultiInsertSync(queryString);
								}
								if (newCgwData.recursos[i].radio.colateral.emplazamientos[j].uriRxA != '') {
									queryString = 'INSERT INTO lista_uris (recurso_radio_id,uri,tipo,nivel_colateral) ' +
										'VALUES (' + resRadioId[k] + ',' +
										'"' + newCgwData.recursos[i].radio.colateral.emplazamientos[j].uriRxA + '"' +
										',"RXA",' + j + 1 + ')';
									res = await
									ug5kdb.QueryMultiInsertSync(queryString);
								}
								if (newCgwData.recursos[i].radio.colateral.emplazamientos[j].uriRxB != '') {
									queryString = 'INSERT INTO lista_uris (recurso_radio_id,uri,tipo,nivel_colateral) ' +
										'VALUES (' + resRadioId[k] + ',' +
										'"' + newCgwData.recursos[i].radio.colateral.emplazamientos[j].uriRxB + '"' +
										',"RXB",' + j + 1 + ')';
									res = await
									ug5kdb.QueryMultiInsertSync(queryString);
								}
			...
			//Enganchar recurso radio antiguo con el nuevo insertado
			async function insertGtwRadioBlackWhiteUriListData(newInsertCgwId, flagResources, resRadioId, resPhoneId, callback) {
				var queryString = '';
				var k=0;
				if(flagResources.resRadio) {
					for (var i=0;i<newCgwData.recursos.length;i++) {
						if(newCgwData.recursos[i].Radio_o_Telefonia==1) {
							...
							for (var j = 0; j < newCgwData.recursos[i].blanca.length; j++) {
			...
			//Enganchar recurso radio antiguo con el nuevo insertado
			async function insertGtwPhoneATSRangeData(newInsertCgwId, flagResources, resRadioId, resPhoneId, callback) {
				var queryString = '';
				var k=0;
				if(flagResources.resTfno) {
					for (var i=0;i<newCgwData.recursos.length;i++) {
						if(newCgwData.recursos[i].Radio_o_Telefonia==2) {
							...
							for (var j = 0; j < newCgwData.recursos[i].telefonia.ats_rangos_dst.length; j++) {

...
bool FileSupervisor::LocalLock(string pathoffile)
{
	...
	file->LocalLock(LocalConfig::SupervisedFileUnlocktime());
...
void FileSupervisor::Init()
{
	try
	{
		INISection SupervisedFiles = LocalConfig::cfg.getSection(strFilesSupervidor);
...
{
	string httpHost = ipRemote + ":" + LocalConfig::cfg.PuertoEscucha();

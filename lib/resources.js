...
exports.getRadioWBList = function getRadioWBList(rsc,listType,f){
	...
	connection.connect(function(err){
		if (err){
			...
			//Metemos el alias y el id para mantener la compatibilidad al mostrar los datos en la tabla del cliente
			//ShowRangeAts(data); en hardware.js
			var query = connection.query('SELECT uri '+
				'FROM lista_uris '+
				'WHERE recurso_radio_id=? AND tipo=?',[rsc,listType],
				function(err, rows) {
					logging.LoggingDate(query.sql);
					connection.end();
...
exports.getRemoteRadioResources = function getRemoteRadioResources(cfg,site,gtw,f){
	...
	connection.connect(function(err) {
		if (err) {
			...
			var query = connection.query('SELECT nombre as name ' +
				'FROM emplazamientos ' +

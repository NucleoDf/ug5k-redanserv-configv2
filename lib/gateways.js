		...
		if (err){
			...
			if (cfg == null){ //Venimos de ver la gtw al pulsar sobre ella
				query = connection.query('SELECT p.idpasarela as idCGW, p.emplazamiento_id as EMPLAZAMIENTO_idEMPLAZAMIENTO,  '+
										...
										'p.snmpv2, p.nombre_snmp, p.localizacion_snmp, contacto_snmp, p.puerto_servicio_web, '+
										'p.tiempo_sesion, p.puerto_rtsp, p.servidor_rtsp '+
										'FROM pasarelas p '+
...
exports.getIpList4Gateway = function delGateway(gtw, f) {
	var mySql = require('mySql');
	var connection = mySql.createConnection({
		host: gcfg.Ulises.dbhost,
		user: gcfg.Ulises.dbuser,
	...
	connection.connect(function (err) {
		if (err) {
			...
			var query = "";
			query = connection.query('SELECT * FROM lista_ips ' +
				'WHERE pasarela_id = ?', gtw,
				function (err, rows, fields) {
					logging.LoggingDate(query.sql);
					if (err)
						f(err.message);
...
exports.delGateway = function delGateway(req, res, gtw){
	...
	connection.connect(function(err){
		if (err){
			...
			/** 20170522. AGL. Quitamos la restriccion de Pasarela en CFG Activ*/
			var querySel = connection.query('SELECT c.idCGW FROM CGW c WHERE idCGW=? ', gtw,
				function(err, rows) {

...
/********************************/
/*	FUNCTION: getGatewaysOnDB */
/*  PARAMS: 					*/
/*  REV 1.0.2 VMG				*/
/********************************/
exports.getGatewaysOnDB = function getGatewaysOnDB(req, res, cfg, gtw) {
	var gtws = [];
	var mySql = require('mySql');
	var connection = mySql.createConnection({
		host: gcfg.Ulises.dbhost,
	...
	connection.connect(function (err, usrs) {
		if (err) {
			...
			var query = connection.query('SELECT p.nombre as name,p.ip_cpu0,p.ip_cpu1,p.ip_virtual as ipv '+
					...
					'WHERE c.activa=1 AND c.idconfiguracion=?',[cfg],
			function(err, rows, fields) {
				connection.end();
				logging.LoggingDate(query.sql);
				...
				if (err){
					...
					if (rows == null || rows.length == 0) {//No en BBDD
						...
						if(rows[0].activa==0)//No en config Activa
							...
							f({error:null, toLocal:null, data:{idConf:rows[0].idConf, fechaHora:rows[0].fechaHora+' UTC'}, ipv:-1})
					}
				}
			});
		}

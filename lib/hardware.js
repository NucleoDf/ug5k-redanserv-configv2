...
multipleStatements: true
	...
	connection.connect(function(err){
		if (err){
			...
			function selectRadioRes(callback) {
			...
			function selectPhoneRes(callback) {
			...
			function updateRadioRes(callback) {
			...
			function updatePhoneRes(callback) {
			...
			async function updateOldRadioRes(callback) {
				var query='';
			...
			async function updateOldPhoneRes(callback) {
				var query='';
	...
	if(position.colOrig==position.colDest&&position.rowOrig==position.rowDest)
		...
		connection.connect(function(err){
			if (err) {
				...
				function selectIfResInPosition(callback) {
					...
					function(err, rows) {
						logging.LoggingDate(queryrr.sql);
						if (err) {
							...
							function(err, rows) {
								logging.LoggingDate(queryrt.sql);
								if (err) {
									return callback(err.message);
								}
								else if(rows.length == 0) {
				...
				async function updateNewResPosition(callback) {
					var stringUpdate='';
					if(position.type==1)
				...
				async function updateOldResPosition(callback) {
					if(oldRes.resType==0)
						return callback(null);
					else {
						var stringUpdate = '';
